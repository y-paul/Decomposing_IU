...
transformed parameters {
  ...
  gamma_ex = Phi( gam_mu_phi[1] + gam_phi[,1] ) * 5;
  gamma_de = Phi( gam_mu_phi[2] + gam_phi[,2] ) * 5;
  ...
  for(i in 1:n) {
    ...
    w_pa[i,1] = pwf(pa[i,1], gamma_de[ sub[i] ]) ;
    w_pa[i,2] = pwf(pa[i,2], gamma_de[ sub[i] ]) ;
    ...
    w_pb[i,1] = pwf(pb[i,1], gamma_de[ sub[i] ]) ;
    w_pb[i,2] = pwf(pb[i,2], gamma_de[ sub[i] ]) ;
    ...
    w_fa[i,1] = pwf(fa[i,1], gamma_ex[ sub[i] ]) ;
    w_fa[i,2] = pwf(fa[i,2], gamma_ex[ sub[i] ]) ;
    ...
    w_fb[i,1] = pwf(fb[i,1], gamma_ex[ sub[i] ]) ;
    w_fb[i,2] = pwf(fb[i,2], gamma_ex[ sub[i] ]) ;
...
model {
  ...
  L_gam_omega ~ lkj_corr_cholesky(4);
...
generated quantities {
  ...
  // transform the means
  real mu_gamma_ex = Phi(gam_mu_phi[1]) * 5;
  real mu_gamma_de = Phi(gam_mu_phi[2]) * 5;
  ...
  real sig_gamma_ex = gam_sigma[1];
  real sig_gamma_de = gam_sigma[2];
